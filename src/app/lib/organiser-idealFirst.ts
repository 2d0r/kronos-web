...
export async function organiseTimespan(timespan: [Date, Date]) {
    ...
    // Calculate priority on the earliest gap
    // Loop through tasks from highest score
        // Place the task in its ideal spot (frequency, time of day, day of week, deadline vs % left)
        // If repeating: Go to its next occurrence
            // Gap or replace: 
                // if there is an ideal gap right there, schedule it
                // if not, compare with the tasks occupying the best gaps, in order
                    // if any comes out with lower score, replace
                        // for the displaced task.. find its nearest gaps OR remove event and bring back into backlog?
                    // if not, seek farther gaps
        // If one-time, move cursor right after and loop

            // 
            // If there is an overlap - compare with that task
                // If new one wins, takes its exact spot and pushes the other one. If not, other way round.
                    // whichever loses, moves
                    // spot its nearest ideal spots
                    // if one is free, move there; if not, move to the nearest gap
                // 
